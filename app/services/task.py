...
from app.services import llm, material, image_material, subtitle, video, voice
...
def get_video_materials(task_id, params, video_terms, audio_duration):
    if params.video_source == "local":
    ...
    elif getattr(params, 'background_media_type', 'video') == 'image':
        logger.info(f"\n\n## downloading images from {params.image_provider}")
        downloaded_images = image_material.download_images(
            task_id=task_id,
            search_terms=video_terms,
        ...
        if not downloaded_images:
            sm.state.update_task(task_id, state=const.TASK_STATE_FAILED)
            logger.error(
                "failed to download images. Check your API keys and network."
            )
            return None
...
def generate_final_videos(
    ...
    for i in range(params.video_count):
        ...
        is_image_mode = getattr(params, 'background_media_type', 'video') == 'image'
