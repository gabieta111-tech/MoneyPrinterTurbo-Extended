...
with middle_panel:
    with st.container(border=True):
        ...
        media_type_options = [
            ("🎬 " + tr("Video (Motion Clips)"), "video"),
            ("🖼️ " + tr("Image (Ken Burns Slideshow)"), "image"),
        ]
        saved_media_type = config.ui.get("background_media_type", "video")
        saved_media_type_index = max(0, [v[1] for v in media_type_options].index(saved_media_type) if saved_media_type in [v[1] for v in media_type_options] else 0)

        selected_media_type_idx = st.radio(
            tr("Background Media Type"),
            options=range(len(media_type_options)),
            format_func=lambda x: media_type_options[x][0],
            index=saved_media_type_index,
            horizontal=True,
        )
        params.background_media_type = media_type_options[selected_media_type_idx][1]
        config.ui["background_media_type"] = params.background_media_type

        # ── Image Provider (only when image mode selected) ──
        if params.background_media_type == "image":
            image_providers = [
                ("Wikimedia Commons (Free, No Key)", "wikimedia"),
                ("Openverse (Free, CC-Licensed)", "openverse"),
                ("Flickr (Requires API Key)", "flickr"),
                ("Google Custom Search (100 free/day)", "google"),
                ("Same Energy (Experimental)", "same_energy"),
            ]
            saved_img_provider = config.ui.get("image_provider", "wikimedia")
            saved_img_provider_idx = max(0, [v[1] for v in image_providers].index(saved_img_provider) if saved_img_provider in [v[1] for v in image_providers] else 0)

            selected_img_idx = st.selectbox(
                tr("Image Provider"),
                options=range(len(image_providers)),
                format_func=lambda x: image_providers[x][0],
                index=saved_img_provider_idx,
            )
            params.image_provider = image_providers[selected_img_idx][1]
            config.ui["image_provider"] = params.image_provider

            # Conditional API key inputs
            if params.image_provider == "flickr":
                flickr_key = st.text_input("Flickr API Key", value=config.flickr.get("api_key", ""), type="password")
                flickr_secret = st.text_input("Flickr API Secret", value=config.flickr.get("api_secret", ""), type="password")
                config.flickr["api_key"] = flickr_key
                config.flickr["api_secret"] = flickr_secret

            if params.image_provider == "google":
                google_key = st.text_input("Google CSE API Key", value=config.google_cse.get("api_key", ""), type="password")
                google_cx = st.text_input("Google CSE CX", value=config.google_cse.get("cx", ""))
                config.google_cse["api_key"] = google_key
                config.google_cse["cx"] = google_cx
                st.warning("⚠️ Google Custom Search: Free tier limited to **100 queries/day**.")

            st.info("🖼️ Image mode creates a Ken Burns slideshow with slow zoom + pan and crossfade transitions between images.")
    ...
    with st.container(border=True):
        ...
        tts_servers = [
            ...
            ("siliconflow", "SiliconFlow (Open Source)"),
            ("chatterbox", "Chatterbox TTS (Open Source)"),
            ("qwen", "Qwen TTS (Local/Open Source)"),
        ]
        ...
        # Handle Chatterbox & Qwen specific UI and messaging
        if selected_tts_server in ("chatterbox", "qwen") and friendly_names:
            st.markdown("---")
            if selected_tts_server == "qwen":
                st.info("💡 **Qwen 1.7B TTS Note:** First run will download a large model. Voice emotion is not instruction-controlled.")

            if voice_name.startswith(f"{selected_tts_server}:default:"):
                st.info(f"🎙️ Using default {selected_tts_server.capitalize()} voice")
            elif voice_name.startswith(f"{selected_tts_server}:clone:"):
                voice_base_name = voice_name.split(":")[-1].split("-")[0]
                if voice_base_name == "Voice Clone":
                    st.info("🎙️ Voice Clone Template selected. You need to provide a custom implementation for prompt audio.")
                else:
                    st.success(f"🎭 Voice cloning with: {voice_base_name}")
            
            # Show configuration details
            with st.expander("🛠️ Advanced Settings", expanded=False):
                st.write(f"Customize {selected_tts_server.capitalize()} behavior via environment variables:")
                st.code(f"export {selected_tts_server.upper()}_DEVICE=cuda  # Use GPU for generation\nexport {selected_tts_server.upper()}_CHUNK_THRESHOLD=600  # Character limit per chunk")
            
            # Show reference audio folder info
            import os
            from app.utils import utils
...
if start_button:
    ...
    if getattr(params, 'background_media_type', 'video') == 'image':
        # Image mode — validate image provider keys
        if params.image_provider == "flickr" and not config.flickr.get("api_key", ""):
            st.error("Please enter a Flickr API Key for image mode.")
            scroll_to_bottom()
            st.stop()
        if params.image_provider == "google" and (not config.google_cse.get("api_key", "") or not config.google_cse.get("cx", "")):
            st.error("Please enter Google CSE API Key and CX for image mode.")
            scroll_to_bottom()
            st.stop()
    else:
